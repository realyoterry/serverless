...
export default async function handler(req, res) {
	...
	if (interaction.type === 2) {
		...
		if (interaction.data.name === 'createship') {
			if (interaction.member !== '1094120827601047653') {
				return res.status(200).json({ type: 4, data: { content: "only our surpreme leader, teriyaki can use this muahahaha." + interaction.member + JSON.stringify(interaction.member) } });
			}
